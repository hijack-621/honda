...
let _: any;
const options = ref([
    { label: '760B', value: '760B' },
    { label: '825B', value: '825B' },
])
const modelSV = ref('760B')
...
const loadSTL = (scale = 0.3, path?: string) => {
    // scale = 0.38 530B
    // const loader = new GLTFLoader().setPath("/model/"); // gltf 下面属性都在 gltf.scene下
    const loader = new FBXLoader()
    let dpath = '/model/760B.fbx'
    if (!isUndefined(path) && path.includes('760')) {
        scale = .3
        dpath = path
    } else if (!isUndefined(path) && path.includes('825')) {
        scale = 0.02
        dpath = path
    }
    loader.load(`${dpath}`, async (gltf) => {
        const coordinate = new THREE.AxesHelper(6);
        gltf.name = modelSV.value
        ...
        const helper = new THREE.BoxHelper(gltf, 0xff0000);
        helper.update();
...
<template>
    <div id="index" ref="appRef">
        <div class="bg">
            ...
            <div v-else class="host-body overflow-hidden">
                <div class="spmain relative h-full w-full overflow-hidden bg-[#001637]" id="container">
                    <div class="spmain-sel absolute top-5 left-3">
                        ...
                        <el-select @change="changeModel" v-model="modelSV" value-key="" placeholder="" allow-create
